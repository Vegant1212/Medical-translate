...
export async function transcribeMedia(
  ...
  // Speech-to-text providers accept audio payloads, not video containers.
  // Always extract the audio track first so MP4/WebM uploads do not fail at
  // the network layer and so the upload progress reflects the actual payload.
  onProgress?.(isVideo ? "Preparando audio del vídeo…" : "Preparando audio…");
